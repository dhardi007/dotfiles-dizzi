...

{
  ...
  home.activation.guide = config.lib.dag.entryAfter [ "writeBoundary" ] ''
    ...
    echo "  Cuando PipeWire levante, crea la fuente 'Noise Canceling source'."
    echo "  No hay nada que abrir; el plugin se carga al arrancar el servidor."
    echo "  En tu app (Meet, Discord, OBS): Ajustes → Audio → Micrófono,"
    echo "  elegí 'Noise Canceling source' en vez de tu micrófono físico."
    echo "  ═ upgrade ★★★ (opcional): Khip en nixos/pkgs/ o nvidia-voice-ai;"
    echo "  ═ ver acotaciones en nixconf/nixos/features/pipewire.nix"
    echo "════════════════════════════════════════════════════════════════"
    ...
    echo "     AUTO. Se registra como servicio de Windows: corre solo y"
    echo "     cubre todos los juegos de ESE prefijo. Nada más que hacer"
    echo "     en ~/.wine. 1 .desktop para lanzarlo a mano (vía script"
    echo "     rpc-bridge.sh) si el servicio no arrancó:"
    echo "     - wine-discord-ipc-bridge.desktop → Wine + Bottles a la vez"
    echo "     BOTTLES: prefijo distinto → hay que instalar bridge.exe"
    echo "     → Install dentro de cada botella (override Flatpak ya OK)."
    echo
    echo "  2) wine-discord-ipc-bridge (0e4ef622) ← NO usado"
    echo "     MANUAL. No es servicio, no es automático. Steam: por juego,"
    echo "     Launch Options '.../winediscordipcbridge-steam.sh %command%'."
    echo "     Wine/Lutris no-Steam: lanzar winediscordipcbridge.exe ANTES"
    echo "     del juego, en el mismo prefijo. 2 .desktop para eso:"
    echo "     - wine-discord-ipc-bridge-wine.desktop    → prefijo ~/.wine"
    echo "     - wine-discord-ipc-bridge-bottles.desktop → botella gaming"
    echo "════════════════════════════════════════════════════════════════"
    echo "  BOTTLES: ¿GE-Proton o wine-ge-proton? (detalle en install-bottles.sh)"
    echo
    echo "  wine-ge-proton (Wine-GE) ← runner por defecto"
    echo "    Wine clásico con parches GE. Prefix propio por botella +"
    echo "    componentes libres (dotnet48, dxvk, vkd3d, winetricks). →"
    echo "    juegos no-Steam, apps, instaladores, emuladores, clásicos y"
    echo "    TODOS los que usan mods/.NET en el prefix (HK/Silksong+BepInEx)."
    echo
    echo "  GE-Proton (Proton-GE) ← solo cuando Wine-GE no alcanza"
    echo "    Fork de Proton (Valve)+GE: llega como 'custom tool' (ge-proton*)"
    echo "    con script proton/protonfixes. Menos control del prefix. →"
    echo "    AAA/online recientes que exigen los últimos parches de Proton"
    echo "    y fallan con Wine-GE (ej. Sparking Zero)."
    echo
  ...
  home.activation.antigravityWritable = config.lib.dag.entryAfter [ "writeBoundary" ] ''
    ...
    # Step 1: Check if already copied and marker exists
    if [ -d "$RUNTIME" ] && [ -f "$MARKER" ] && [ "$(cat "$MARKER" 2>/dev/null)" = "$ANTIGRAVITY_SOURCE" ]; then
      ...
      # Step 2: Fix read-only EROFS by replacing all store paths
      grep -rIl "$ANTIGRAVITY_SOURCE" "$RUNTIME" 2>/dev/null | while read -r f; do
        sed -i "s|$ANTIGRAVITY_SOURCE|$RUNTIME|g" "$f"
      done
    
      # Step 3: Ensure extensions dir is writable
      mkdir -p "$RUNTIME/lib/antigravity-ide/resources/app/extensions"
      chmod -R u+w "$RUNTIME/lib/antigravity-ide/resources/app/extensions" 2>/dev/null || true
    
      # Step 4: Create a tmp dir for lock files (workaround EROFS)
      mkdir -p "$RUNTIME/lib/antigravity-ide/resources/app/out"
      chmod -R u+w "$RUNTIME/lib/antigravity-ide/resources/app/out" 2>/dev/null || true
